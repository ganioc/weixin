...
function get_user_games(req,res, callback){
  db.users.findOne(
    ...
    function(err,doc){
      ...
      var names = doc.games;// which is an Array
      var l = [];
      for(var i in names){
	if(!( names[i].name in l)){
	  l.push( names[i].name );
	}
      }
      console.log('list is:');
...
function get_games(req,res){
  ...
  get_user_games(req, res, function(userGames){
    console.log('usergames is:');
    console.log(userGames);

    var games1 = userGames;//用户所拥有的游戏列表
...
function get_code(req,res){
  ...
  url += 'appid=' + config.TEST_APPID;
  url += '&redirect_uri=' + config.TEST_URI;
