...
var collections = ['users','games','codes','articles', 'comments'];
...
module.exports= {
  ...
  init: function(app)
  {
    ...
    // use flash to store temperary names or variables
    app.use(flash());
  ...
  callbackUpdate:function(callback, callbackFail){
    
    return function(err, doc){
      console.log('callbackUpdate');
      console.log(doc);
      if(err){
	console.log('db update failure, serious!');
	throw err;
      }
      if(doc.nModified !== 0 || doc.upserted.length >0){
  ...
  callbackSave: function(callback){
    return function(err,doc){
      if(err){
	console.log('db save failed, serious err!');
	throw err;
      }
      if(doc){
	console.log(doc);
